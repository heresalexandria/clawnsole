...
Below the header, a single-line toolbar contains Copy, AI rewrite, and Characters.
...
headings, speakers, uppercase action entities, and assigned characters. Tap a
...
The editor centers a screenplay column on desktop and scales its on-screen
margins to the viewport, while retaining compact indentation in plain text. This is
...
The **Characters** modal lists detected script characters and casting lines.
...
Providers receive their existing reference dialects (for example `@video1` for
Seedance adapters). Character assignments are authoring metadata and introduce
no provider-specific fields, privileged renderer calls, or inline media.
